...
while ($рПередача = stream_socket_accept($рПриёмник, -1))
	...
	if(isset($мЗаголовкиСлушателя[0]))
		...
		elseif(isset($мЗаголовки[1])&&$мЗаголовки[1]=="/favicon.ico")
			{
			$спИконка			=сПостроитьПакетДанныхЛоготипИконка();
			fwrite($рПередача, $спИконка);
			unset($спИконка);
			fclose($рПередача);
			}
		elseif(isset($мЗаголовки[1])&&$мЗаголовки[1]=="/robots.txt")
			{
